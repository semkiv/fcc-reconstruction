...
def process(file_name, max_events, n_bins, x_min, x_max, fit, peak_x_min, peak_x_max, verbose):
    ...
    if fit:
        ...
        # Bs background model: Gaussian + Crystal Ball shape
        mean_bs = RooRealVar('mean_bs', '#mu_{Bd}', 4.980)
        width_bs_gauss = RooRealVar('width_bs_gauss', '#sigma_{Bs Gauss}', 0.063)
        ...
        mean_bd = RooRealVar('mean_bd', '#mu_{Bd}', 4.893)
        width_bd_gauss = RooRealVar('width_bd_gauss', '#sigma_{Bd Gauss}', 0.356)
...
def main(argv):
    ...
    process(args.ifile, max_events, NBINS, XMIN, XMAX, args.fit, PEAK_MIN, PEAK_MAX, args.verbose)
